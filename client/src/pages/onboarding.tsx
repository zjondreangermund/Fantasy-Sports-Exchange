            ...
            {packs.map((pack, packIndex) => {
              ...
              return (
                <section key={packIndex} className={`overflow-hidden rounded-[1.5rem] border bg-white/[0.045] shadow-[0_16px_50px_rgba(0,0,0,.2)] transition ${hasSelectionInPack ? "border-cyan-300/35" : "border-white/10"}`}>
                  ...
                  <div className="grid grid-cols-3 items-start gap-1.5 p-2 sm:gap-4 sm:p-5">
                    {pack.map((card, cardIndex) => {
                      ...
                      return (
                        <motion.button
                          ...
                          className={`relative flex min-w-0 flex-col items-center rounded-2xl border p-1.5 text-center outline-none transition focus-visible:ring-2 focus-visible:ring-cyan-300 sm:p-3 ${isSelected ? "border-cyan-300/60 bg-cyan-300/10 shadow-[0_0_28px_rgba(34,211,238,.16)]" : "border-white/8 bg-black/20 hover:border-white/20 hover:bg-white/[0.04]"}`}
                          whileTap={{ scale: 0.98 }}
                        >
                          <span className="absolute left-2 top-2 z-20 grid h-6 w-6 place-items-center rounded-full border border-white/15 bg-slate-950/85 text-[10px] font-black text-white/65">{cardIndex + 1}</span>
                          {isSelected ? <span className="absolute right-2 top-2 z-20 grid h-7 w-7 place-items-center rounded-full bg-cyan-300 text-slate-950 shadow-lg"><Check className="h-4 w-4" /></span> : null}
                          <div className="w-full min-w-0 overflow-hidden"><CardThumbnail card={card} size="sm" selected={isSelected} selectable /></div>
                          <span className={`mt-2 rounded-full px-2 py-1 text-[9px] font-black uppercase tracking-[0.12em] ${isSelected ? "bg-cyan-300 text-slate-950" : "bg-white/[0.06] text-white/45"}`}>{isSelected ? "Your pick" : "Tap to select"}</span>
                        </motion.button>
                      );
                    })}
                  </div>
