...
const documents: Record<string, Document> = {
  "/legal/terms": {
    ...
    sections: [
      ...
      { title: "Wallets, payments and withdrawals", bullets: ["A deposit remains pending until Fantasy Arena verifies the payment reference. The wallet is credited only after approval.", "Deposits below N$200 carry a 2% fee; deposits of N$200 or more are fee-free.", "The minimum withdrawal is N$20. Fantasy Arena charges no withdrawal fee.", "The requested amount is moved from available balance to locked balance while it is reviewed.", "A bank, eWallet or payment gateway may apply its own external processing fee.", "Paid withdrawals release the approved amount. Rejected withdrawals return the full held amount to available balance.", "Fantasy Arena may request verification before withdrawals, refunds, high-risk transactions or where required by a provider or law."] },
  ...
  "/legal/game-rules": {
    ...
    sections: [
      ...
      { title: "Settlement and winner messages", bullets: ["After final scoring and settlement, winning users receive a congratulations message from the Fantasy Arena Team in their inbox.", "Cash tournament payouts are credited to the Fantasy Arena wallet during settlement.", "Gameweek reminders and deadline notices are delivered through the notification inbox instead of large pop-ups.", "Unread notifications are marked with a purple indicator. The indicator disappears when the notification is read or all notifications are marked read."] },
  ...
  "/legal/refunds": {
    ...
    sections: [
      ...
      { title: "Withdrawals", bullets: ["The minimum withdrawal is N$20.", "Fantasy Arena charges no withdrawal fee.", "When submitted, the full amount moves from available balance to locked balance so it cannot be spent twice.", "The request is reviewed and may require identity, ownership or fraud checks.", "Once approved and paid, the requested amount is sent to the selected bank, eWallet or supported destination and a payout reference is recorded.", "The receiving bank, eWallet or payment gateway may deduct its own processing fee.", "If rejected, the full held amount returns to available balance. A failed payout remains held for an authorised retry or refund."] },
    ],
  },
  ...
  "/legal/scoring": {
    title: "Scoring Rules",
    intro: "Fantasy Arena scores five-card lineups using real Premier League match statistics and the scoring model published in the platform.",
    sections: [
      { title: "Scoring process", bullets: ["Player events and statistics are received from official third-party football-data providers.", "Each card receives an official score based on minutes, decisive actions, all-around actions and applicable bonuses or penalties.", "The captain receives a 10% lineup bonus. Rarity never multiplies football points."] },
      { title: "Corrections", bullets: ["Live scores are provisional.", "Official data corrections may change player scores and rankings before settlement.", "The final gameweek scoring snapshot stored by Fantasy Arena is the controlling calculation for settlement."] },
    ],
  },
};
...
const generalPages: Record<string, Document> = {
  ...
  "/faq": { title: "Frequently Asked Questions", intro: "Quick answers to common Fantasy Arena questions.", sections: [{ title: "Do I need to verify immediately?", paragraphs: ["Not normally. Fantasy Arena uses tiered, risk-based verification and may request documents for withdrawals, prize claims, higher-risk activity or provider requirements."] }, { title: "How many cards do I need?", bullets: rarityEntryRules }, { title: "Can I enter more than once?", paragraphs: ["Yes, where the tournament allows multiple entries. Every team is charged separately and must use five different, unused cards."] }, { title: "How are points calculated?", paragraphs: ["Points come from real Premier League match statistics. The captain receives a 10% lineup bonus. Live points are provisional and can change after official corrections."] }, { title: "How do deposits and withdrawals work?", paragraphs: ["Deposits are verified before wallet credit. The minimum withdrawal is N$20 and Fantasy Arena charges no withdrawal fee. The requested amount is locked during review; a bank, eWallet or payment gateway may apply its own external fee."] }, { title: "How do Prize Vault rewards unlock?", paragraphs: ["Each rarity ladder uses its own entry count and funding target. The highest fully unlocked reward becomes the active prize for that gameweek."] }] },
};
