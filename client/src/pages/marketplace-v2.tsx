...
export default function MarketplaceV2Page() {
  ...
  const [watchlist, setWatchlist] = useState<number[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("market_watchlist_card_ids") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const setMode = (mode: MarketMode) => {
    ...
    if (typeof window !== "undefined") {
      window.history.replaceState(null, "", mode === "loan" ? "/marketplace?mode=loan" : "/marketplace");
    }
  };
  ...
  return (
    <LivePageShell tone="trading">
      ...
      <section className="grid gap-4 lg:grid-cols-[0.78fr_1.22fr]">
        <Card className="cinematic-glass border-cyan-300/15 bg-white/[0.06] p-5 text-white backdrop-blur-xl">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-black">Market Pulse</h2>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <Pulse label="Wallet" value={money(walletBalance)} icon={<WalletCards className="h-4 w-4" />} />
          </div>
        </Card>
        ...
        <Card className="cinematic-glass border-white/10 bg-slate-950/60 p-5 text-white backdrop-blur-xl">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-black">Hot Board</h2>
          </div>
          <div className="grid gap-2 sm:grid-cols-3">
            {hotCards.length ? hotCards.map((card) => (
              <HotCard key={cardId(card)} card={card} onClick={() => setSelected(card)} />
      ...
      <section className="rounded-[2rem] border border-white/10 bg-black/35 p-4 shadow-[0_24px_80px_rgba(0,0,0,.35)] backdrop-blur-2xl">
        <div className="mb-4 grid gap-3 lg:grid-cols-[auto_1fr_auto_auto] lg:items-center">
          <div className="grid grid-cols-3 gap-1 rounded-2xl border border-white/10 bg-black/45 p-1">
            ...
            <Button
              size="sm"
              variant={marketMode === "loan" ? "default" : "ghost"}
              onClick={() => setMode("loan")}
              className="gap-2 rounded-xl"
            >
              <Handshake className="h-4 w-4" /> Loan
            </Button>
            <Button asChild size="sm" variant="ghost" className="gap-2 rounded-xl">
              <Link href="/auctions"><Gavel className="h-4 w-4" /> Auction</Link>
            </Button>
          </div>
          ...
          <select
            ...
            className="h-12 rounded-xl border border-white/10 bg-black/45 px-3 text-sm text-white outline-none"
          >
        ...
        {marketMode === "loan" ? (
          <LoanMarketPanel
            myCards={normalizeCards(myCards)}
            ...
            {isLoading ? (
              ...
              filtered.map((card) => (
                <MarketRow
                  key={cardId(card)}
                  card={card}
                  watched={watchlist.includes(cardId(card))}
                  onWatch={() => toggleWatch(cardId(card))}
                  onBuy={() => setBuying(card)}
                  onDetails={() => setSelected(card)}
                />
              ))
            ) : (
              <Card className="border-slate-800 bg-slate-950/60 p-12 text-center">
                ...
                <p className="text-lg text-slate-300">No cards match your search.</p>
              </Card>
            )}
          </div>
        )}
      ...
      <Dialog open={!!buying} onOpenChange={(open) => !open && setBuying(null)}>
        <DialogContent className="border-white/10 bg-[#070b18] text-white shadow-[0_30px_100px_rgba(0,0,0,.75)]">
          <DialogHeader><DialogTitle>Confirm Purchase</DialogTitle></DialogHeader>
          {buying ? (
            <div className="py-4">
              <div className="mb-4 flex justify-center"><CardShowcase card={buying} size="sm" /></div>
              <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
                <p>Buy <strong>{buying.player?.name}</strong> for <strong className="text-emerald-300">{money(cardPrice(buying))}</strong>?</p>
                <p className="mt-1 text-sm text-slate-400">Seller: {ownerName(buying)}</p>
                <p className="mt-2 text-sm text-slate-400">Your Balance: {money(walletBalance)}</p>
                {walletBalance < cardPrice(buying) ? <p className="mt-2 text-sm font-bold text-red-300">Insufficient wallet balance.</p> : null}
              </div>
            </div>
          ) : null}
          <DialogFooter>
            ...
            <Button
              disabled={!buying || buyMutation.isPending || walletBalance < cardPrice(buying)}
              onClick={() => buying && buyMutation.mutate(buying)}
            >
...
function Pulse({ label, value, icon }: { label: string; value: string; icon: ReactNode }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-black/25 p-3">
      <div className="mb-2 flex h-9 w-9 items-center justify-center rounded-xl bg-cyan-400/10 text-cyan-300">{icon}</div>
...
function HotCard({ card, onClick }: { card: PlayerCardWithPlayer; onClick: () => void }) {
  const fantasy = toFantasyCardData(card, { imageWidth: 320 });
  return (
    <button onClick={onClick} className="rounded-2xl border border-white/10 bg-black/25 p-3 text-left transition hover:bg-cyan-400/10">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <Badge className="mt-1 bg-emerald-400 text-black">{money(cardPrice(card))}</Badge>
        </div>
      </div>
    </button>
  );
...
function MarketRow({
  ...
  const fantasy = toFantasyCardData(card, { imageWidth: 420 });
    ...
    >
      ...
      <div className="relative grid gap-3 md:grid-cols-[1.45fr_0.7fr_0.7fr_0.9fr_auto] md:items-center">
        ...
        <div className="rounded-xl border border-emerald-300/20 bg-emerald-400/10 px-3 py-2">
          <p className="text-[10px] uppercase tracking-[.14em] text-emerald-100/60">Buy Now</p>
          <p className="text-lg font-black text-emerald-300">{money(price)}</p>
        </div>
