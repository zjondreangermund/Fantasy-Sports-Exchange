...
const rarityTheme: Record<string, { accent: string; glow: string; panel: string; gradient: string }> = {
  common: { accent: "#60a5fa", glow: "rgba(59,130,246,.45)", panel: "#071221", gradient: "from-blue-500/25 via-slate-900/70 to-black" },
  rare: { accent: "#22d3ee", glow: "rgba(34,211,238,.48)", panel: "#04151d", gradient: "from-cyan-500/25 via-slate-900/70 to-black" },
  unique: { accent: "#c084fc", glow: "rgba(168,85,247,.5)", panel: "#17061f", gradient: "from-purple-500/30 via-slate-900/70 to-black" },
  epic: { accent: "#fb3b4a", glow: "rgba(251,59,74,.5)", panel: "#210709", gradient: "from-rose-500/30 via-slate-900/70 to-black" },
  legendary: { accent: "#f59e0b", glow: "rgba(245,158,11,.5)", panel: "#211502", gradient: "from-amber-500/30 via-slate-900/70 to-black" },
};
...
export default function CompetitionsVaultPage() {
  ...
  const currentGw = useMemo(() => {
    ...
    const future = official.filter((c) => c.status === "upcoming").map((c) => Number(c.gameWeek || 0)).filter(Boolean).sort((a, b) => a - b);
    return future[0] || 1;
  }, [official]);
  ...
  return (
    <main className="min-h-full overflow-x-hidden bg-[#02040c] px-3 pb-[calc(10rem+env(safe-area-inset-bottom,0px))] pt-4 text-white sm:px-6 lg:px-8">
