...
import { CalendarDays, CheckCircle2, Clock3, Filter, Gift, KeyRound, Lock, Trophy } from "lucide-react";
...
const progress = (comp: Tournament) => { const target = Number(comp.requiredEntrants || 0); return target ? Math.min(100, Math.round((Number(comp.entryCount || comp.entry_count || 0) / target) * 100)) : 0; };
const isOfficial = (comp: Tournament) => comp.ladderLinked === true || comp.rawPrizeKey === "ladder" || comp.prizeKey === "ladder" || comp.prize_key === "ladder" || /Prize Ladder|Official/i.test(String(comp.name || ""));
...
export default function CompetitionsVaultPage() {
  ...
  const official = useMemo(() => competitions.filter((c) => rarityOrder.includes(tier(c.tier)) && isOfficial(c)), [competitions]);
  const currentGw = useMemo(() => {
    const live = official.filter((c) => ["open", "active"].includes(String(c.status))).map((c) => Number(c.gameWeek || 0)).filter(Boolean).sort((a, b) => a - b);
    if (live.length) return live[0];
    const upcoming = official.filter((c) => c.status === "upcoming").map((c) => Number(c.gameWeek || 0)).filter(Boolean).sort((a, b) => a - b);
    return upcoming[0] || 1;
  }, [official]);
  const gameweeks = useMemo(() => [...new Set(official.map((c) => Number(c.gameWeek || 0)).filter(Boolean))].sort((a, b) => a - b), [official]);
  const shownGw = gameweekFilter === "current" ? currentGw : gameweekFilter;
  const visible = official.filter((c) => Number(c.gameWeek) === Number(shownGw) && tier(c.tier) === activeRarity);
  ...
  const findPinMutation = useMutation({
    ...
    onSuccess: (body: any) => { const tournament = body?.tournament; if (!tournament) return; setPinTournament(tournament); setPin(""); },
  ...
  return <main className="min-h-full overflow-x-hidden bg-[#02040c] px-3 pb-[calc(10rem+env(safe-area-inset-bottom,0px))] pt-4 text-white sm:px-6 lg:px-8">
    <div className="mx-auto max-w-7xl space-y-5">
      <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_80%_0%,rgba(124,58,237,.3),transparent_30%),linear-gradient(180deg,#090d20,#040711)] p-5 sm:p-7">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><div className="text-[10px] font-black uppercase tracking-[.28em] text-purple-200/70">Official 2026/27 Arena</div><h1 className="mt-2 text-4xl font-black sm:text-6xl">Rarity Tournaments</h1><p className="mt-2 max-w-3xl text-sm text-white/55">Official Fantasy Arena tournaments are public. Player-created tournaments stay hidden until the correct PIN is entered.</p></div><div className="grid grid-cols-3 gap-2"><Stat icon={CalendarDays} label="Gameweek" value={`GW${shownGw}`} /><Stat icon={Trophy} label="Rarities" value="5" /><Stat icon={Clock3} label="Window" value="Tue–Tue" /></div></div>
      </section>
      <section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-4 sm:p-5">
        ...
        <div className="mt-4 grid gap-2 rounded-2xl border border-purple-300/20 bg-purple-500/10 p-3 sm:grid-cols-[1fr_auto]"><div className="relative"><KeyRound className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-purple-200/60" /><Input value={pin} onChange={(e) => setPin(e.target.value.toUpperCase())} onKeyDown={(e) => { if (e.key === "Enter") findPinMutation.mutate(); }} placeholder="Enter player tournament PIN" className="h-11 border-white/10 bg-black/35 pl-10 uppercase text-white" /></div><Button onClick={() => findPinMutation.mutate()} disabled={findPinMutation.isPending} className="bg-purple-500 font-black hover:bg-purple-400">{findPinMutation.isPending ? "Finding…" : "Find tournament"}</Button></div>
      </section>
