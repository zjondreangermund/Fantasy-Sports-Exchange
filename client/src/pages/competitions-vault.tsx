...
export default function CompetitionsVaultPage() {
  ...
  const activePosition = activeSlot === null ? null : slotDefinitions[activeSlot].position || (currentSlotCard ? playerPosition(currentSlotCard) : null);
  ...
  const activeSlotTitle = activeSlot === null ? "Lineup complete" : activeSlot === 4 && activePosition ? `Replace Utility ${activePosition}` : activeSlot === 4 ? "Choose Utility player" : `Choose ${slotDefinitions[activeSlot].label}`;
      ...
      <div className="mx-auto max-w-7xl space-y-5">
        <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_80%_0%,rgba(124,58,237,.3),transparent_30%),linear-gradient(180deg,#090d20,#040711)] p-5 sm:p-7">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div><div className="text-[10px] font-black uppercase tracking-[.28em] text-purple-200/70">Official 2026/27 Arena</div><h1 className="mt-2 text-4xl font-black sm:text-6xl">Rarity Tournaments</h1><p className="mt-2 max-w-3xl text-sm text-white/55">Every entry uses five Premier League cards. Higher-rarity tournaments require a minimum number of that rarity and allow lower cards in the remaining slots.</p></div>
            <div className="grid grid-cols-3 gap-2"><Stat icon={CalendarDays} label="Gameweek" value={`GW${shownGw}`} /><Stat icon={Trophy} label="Rarities" value="5" /><Stat icon={Clock3} label="Window" value="Tue–Tue" /></div>
          </div>
        </section>
...
function LineupSlotCard({ slotLabel, slotShort, card, active, captain, lockedEmpty, captainEnabled, onOpen, onCaptain }: { slotLabel: string; slotShort: string; card: PlayerCardWithPlayer | null; active: boolean; captain: boolean; lockedEmpty: boolean; captainEnabled: boolean; onOpen: () => void; onCaptain: () => void }) {
  ...
  return <div className={`relative min-w-0 rounded-2xl border p-3 transition ${active ? "border-purple-300/70 bg-purple-500/15" : "border-white/10 bg-white/[0.04]"}`}><button type="button" onClick={onOpen} disabled={lockedEmpty} className="flex w-full min-w-0 items-center gap-3 text-left disabled:cursor-not-allowed disabled:opacity-40"><div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-xl border border-white/10 bg-black/35 font-black text-white/40">{player?.imageUrl ? <img src={player.imageUrl} alt={player.name} className="h-full w-full object-contain object-top" /> : initials}</div><div className="min-w-0 flex-1"><div className="text-[9px] font-black uppercase tracking-[.18em] text-purple-300">{slotShort} • {slotLabel}</div><div className="mt-1 truncate font-black">{player?.name || (lockedEmpty ? "Complete the previous slot" : "Select player")}</div><div className="truncate text-[10px] uppercase tracking-[.12em] text-white/40">{player ? `${player.position} • ${player.team} • ${String(card?.rarity || "")}` : "Premier League"}</div></div>{captain ? <Crown className="h-5 w-5 shrink-0 text-yellow-300" /> : null}</button>{card && captainEnabled ? <button type="button" onClick={onCaptain} className={`mt-3 w-full rounded-xl border px-3 py-2 text-xs font-black ${captain ? "border-yellow-300/50 bg-yellow-300 text-black" : "border-white/10 bg-black/30 text-white/70 hover:border-yellow-300/40 hover:text-yellow-200"}`}>{captain ? "Captain selected" : "Make captain"}</button> : null}</div>;
}
