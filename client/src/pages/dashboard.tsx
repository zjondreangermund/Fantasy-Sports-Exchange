...
export default function DashboardPage() {
  ...
  return (
    <LivePageShell tone="stadium">
      <LiveHero
        eyebrow="Matchday Center"
        title={`Welcome back, ${user?.firstName || "Manager"}`}
        description="Floodlights on. Track your wallet, active tournaments, squad and market moves from one live stadium dashboard."
      >
      ...
      <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <Badge className="gap-1 bg-emerald-500 text-black"><Sparkles className="h-3 w-3" /> Next Move</Badge>
                {notifications?.unreadCount ? <Badge variant="outline" className="border-white/20 text-white">{notifications.unreadCount} unread</Badge> : null}
              </div>
              <h2 className="text-xl font-black text-white sm:text-2xl">{nextAction.title}</h2>
              <p className="mt-1 text-sm text-white/55">Your next best action based on your squad, wallet and current tournament activity.</p>
            </div>
        ...
        <Card className="border-white/10 bg-white/[0.06] p-5 backdrop-blur-xl">
          ...
          {notifications?.notifications?.length ? (
            ...
            <p className="text-sm text-white/50">No important notifications.</p>
          )}
        </Card>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="space-y-4">
          <Card className="border-yellow-400/20 bg-yellow-400/10 p-5 backdrop-blur-xl">
            <div className="mb-4 flex items-center gap-2"><Trophy className="h-5 w-5 text-yellow-300" /><h2 className="font-bold text-white">Active Tournaments</h2></div>
          ...
          <Card className="border-white/10 bg-white/[0.06] p-5 backdrop-blur-xl">
            <div className="mb-4 flex items-center gap-2"><LayoutGrid className="h-5 w-5 text-emerald-300" /><h2 className="font-bold text-white">Collection Snapshot</h2></div>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <ActionCard icon={<ShoppingBag className="h-5 w-5" />} title="Trading Floor" body="Buy cards, manage listings and spot value opportunities." href="/marketplace" cta="Open Market" />
        <ActionCard icon={<Gavel className="h-5 w-5" />} title="Auction Lights" body="Bid on live auction cards and track current offers." href="/auctions" cta="View Auctions" />
      ...
      <section className="grid gap-4 lg:grid-cols-[0.8fr_1.2fr]">
        <Card className="border-white/10 bg-slate-950/55 p-5 backdrop-blur-xl">
          <div className="mb-4 flex items-center gap-2"><Zap className="h-5 w-5 text-emerald-300" /><h2 className="font-bold text-white">Missions</h2></div>
          <div className="space-y-2">
            {(retentionSummary?.missions || []).slice(0, 3).map((mission) => (
              <div key={mission.id} className="flex items-center justify-between rounded-xl border border-white/10 bg-black/25 p-3 text-sm">
                <span className={mission.completed ? "text-green-300" : "text-white"}>{mission.title}</span>
                <Badge variant={mission.completed ? "default" : "outline"}>{mission.progress}/{mission.target}</Badge>
              </div>
            ))}
            {!(retentionSummary?.missions || []).length && <p className="text-sm text-white/50">No missions right now.</p>}
          </div>
        </Card>
        <Card className="border-white/10 bg-slate-950/55 p-5 backdrop-blur-xl">
          <div className="mb-4 flex items-center gap-2"><Zap className="h-5 w-5 text-yellow-300" /><h2 className="font-bold text-white">Deadline Watch</h2></div>
          <div className="rounded-xl border border-white/10 bg-black/25 p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-white/45">Next deadline</p>
            <p className="mt-2 text-sm font-semibold text-white">{retentionSummary?.deadline ? retentionSummary.deadline.competitionName : "No active deadline"}</p>
            <p className="mt-1 text-xs text-white/50">{retentionSummary?.deadline?.startsAt ? new Date(retentionSummary.deadline.startsAt).toLocaleString() : "You are clear for now."}</p>
          </div>
        </Card>
      </section>
    </LivePageShell>
...
function RarityPill({ label, value }: { label: string; value: number }) {
  return <div className="rounded-xl border border-white/10 bg-black/25 p-3"><p className="text-xs uppercase tracking-[0.16em] text-white/45">{label}</p><p className="text-xl font-black text-white">{value}</p></div>;
}
...
function EmptyState({ title, body, action, href, compact = false }: { title: string; body: string; action: string; href: string; compact?: boolean }) {
  return <div className={`rounded-2xl border border-dashed border-white/15 bg-black/20 text-center ${compact ? "p-4" : "p-8"}`}><p className="font-semibold text-white">{title}</p><p className="mx-auto mt-1 max-w-sm text-sm text-white/50">{body}</p><Link href={href}><Button className="mt-4" size="sm">{action}</Button></Link></div>;
}
