...
import { getMarketplaceListingPrice, MARKETPLACE_FLOOR_BY_RARITY, RARITY_ORDER } from "../../../shared/card-economy";
...
type RarityKey = "all" | "common" | "rare" | "unique" | "epic" | "legendary";
const PAGE_SIZE = 10;
const BASE_PRICES: Record<string, number> = { ...MARKETPLACE_FLOOR_BY_RARITY };
const rarityOrder: Record<string, number> = { ...RARITY_ORDER };
...
function cardValue(card: PlayerCardWithPlayer) { return card.forSale ? Math.max(0, getMarketplaceListingPrice(card)) : 0; }
...
export default function CollectionCleanPage() {
  ...
  const { data: cards = [], isLoading } = useQuery<PlayerCardWithPlayer[]>({ queryKey: ["/api/user/cards"], queryFn: async () => { const res = await fetch("/api/user/cards", { credentials: "include" }); if (!res.ok) throw new Error("Failed to fetch cards"); return normalizeCards(await res.json()); }, staleTime: 0, refetchInterval: 15_000, refetchOnWindowFocus: true });
  ...
  const counts = useMemo(() => { const base: Record<string, number> = { common: 0, rare: 0, unique: 0, epic: 0, legendary: 0 }; cards.forEach((card) => { const r = rarityOf(card); if (base[r] !== undefined) base[r] += 1; }); return base; }, [cards]);
  ...
  const filteredCards = useMemo(() => cards.filter((card) => (filter === "all" || rarityOf(card) === filter) && cardMatchesSearch(search, card)).sort((a, b) => (rarityOrder[rarityOf(b)] || 0) - (rarityOrder[rarityOf(a)] || 0) || playerName(a).localeCompare(playerName(b))), [cards, filter, search]);
