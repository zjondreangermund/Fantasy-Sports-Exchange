...
import { LiveHero, LivePageShell } from "../components/layout/LivePageShell";
...
import { Clock, Copy, Crown, Gift, Shield, Sparkles, Trophy, Users, Lock, Flame } from "lucide-react";
...
type CompetitionWithEntries = Competition & {
  ...
  prizeKey?: string;
  prizeValue?: number;
  prizeUnlockTarget?: number;
  requiredEntrants?: number;
  currentEntrantRevenue?: number;
...
function normalizeTier(value: unknown) { return String(value || "common").toLowerCase(); }
function getInviteLink(pin: string) { const base = typeof window !== "undefined" ? window.location.origin : ""; return `${base}/join/${pin}`; }
function tournamentCountdown(comp: CompetitionWithEntries) {
  const raw = comp.submissionClosesAt || (comp as any).endDate || (comp as any).startsAt;
...
export default function CompetitionsPage() {
  ...
  const applyLineupPreset = (ids: unknown, label: string) => {
    ...
    setSelectedCards(validIds); setCaptainId(validIds[0]);
  ...
  const openCompetitionAction = (comp: CompetitionWithEntries, mode: EntryMode = "standard") => {
    ...
    if (comp.entryOpen === false) return toast({ title: "Submission closed", description: "This gameweek is locked at the first Premier League kickoff." });
    setSelectedComp(comp); setEntryMode(mode);
    const savedIds = cardIdsForTier(savedLineup?.lineup?.cardIds, comp.tier);
    const previousIds = cardIdsForTier(previousEntryForTier(comp.tier, comp.id)?.lineupCardIds, comp.tier);
    const startingIds = savedIds.length === 5 ? savedIds : previousIds;
    setSelectedCards(startingIds); setCaptainId(startingIds[0] || null);
  };
  const toggleCard = (cardId: number) => setSelectedCards((prev) => { if (prev.includes(cardId)) { const remaining = prev.filter((id) => id !== cardId); if (captainId === cardId) setCaptainId(remaining[0] || null); return remaining; } if (prev.length >= 5) return prev; const next = [...prev, cardId]; if (!captainId) setCaptainId(cardId); return next; });
  ...
  const joinMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await apiRequest("POST", "/api/competitions/join", { competitionId: selectedComp.id, pin: entryMode === "pin" ? pin : undefined, cardIds: selectedCards, captainId });
      return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/competitions"] }); queryClient.invalidateQueries({ queryKey: ["/api/competitions/my-entries"] }); queryClient.invalidateQueries({ queryKey: ["/api/wallet"] }); setSelectedComp(null); setSelectedCards([]); setCaptainId(null); toast({ title: "Entered tournament!", description: "Your GW points start from 0 and only Premier League matches in this GW count." }); },
    onError: (error: any) => { if (isUnauthorizedError(error)) { toast({ title: "Unauthorized", variant: "destructive" }); setTimeout(() => { window.location.href = "/api/login"; }, 500); return; } toast({ title: "Error", description: error.message, variant: "destructive" }); },
  });
  ...
  const findPinMutation = useMutation({
    mutationFn: async () => { const pin = pinCode.trim().toUpperCase(); if (!pin) throw new Error("Enter a PIN code"); const res = await fetch(`/api/user-tournaments/pin/${encodeURIComponent(pin)}`, { credentials: "include" }); const data = await res.json().catch(() => ({})); if (!res.ok) throw new Error(data?.message || "Tournament not found"); return data.tournament || data; },
    onSuccess: (tournament: CompetitionWithEntries) => setPinTournament(tournament),
    ...
    <LivePageShell>
      ...
      <section className="space-y-4">
        <div className="relative grid gap-3 md:grid-cols-4">
          ...
          <ArenaPulse label="Entrants" value={String(liveComps.reduce((sum, comp) => sum + Number(comp.entryCount || 0), 0))} helper="This gameweek only" icon={<Users className="h-4 w-4" />} />
          <ArenaPulse label="Unlocked prizes" value={`N$${money(totalUnlockedValue)}`} helper={nextUnlock ? `Next: ${nextUnlock.prizeDescription}` : "Prize Vault ready"} icon={<Trophy className="h-4 w-4" />} />
          <ArenaPulse label="Ready cards" value={String(myCommonCards.length)} helper="Common eligible" icon={<Sparkles className="h-4 w-4" />} />
        </div>
        {nextUnlock && <Card className="border-emerald-300/20 bg-emerald-300/10 p-4 text-emerald-50"><div className="flex flex-wrap items-center justify-between gap-3"><div><b>Next prize unlock:</b> {nextUnlock.prizeDescription} needs {nextUnlock.requiredEntrants} entrants. Current: {nextUnlock.entryCount || 0}. Progress {prizeProgress(nextUnlock)}%.</div><Link href="/prize-vault"><Button size="sm" variant="outline" className="border-emerald-200/40 text-emerald-50"><Gift className="mr-2 h-4 w-4" />View Vault</Button></Link></div></Card>}
      </section>
...
function PrizeEconomics({ comp }: { comp: CompetitionWithEntries }) {
  const revenue = n(comp.currentEntrantRevenue) || n(comp.entryFee) * n(comp.entryCount);
  const target = n(comp.prizeUnlockTarget);
...
function ArenaPulse({ label, value, helper, icon }: { label: string; value: string; helper: string; icon: ReactNode }) { return <div className="rounded-3xl border border-white/10 bg-black/30 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,.08)]"><div className="mb-3 flex items-center justify-between"><div className="rounded-2xl border border-cyan-300/20 bg-cyan-300/10 p-2 text-cyan-100">{icon}</div><span className="text-[10px] font-black uppercase tracking-[0.22em] text-white/35">Arena</span></div><div className="text-[11px] uppercase tracking-[0.2em] text-white/45">{label}</div><div className="mt-1 text-2xl font-black text-white">{value}</div><div className="mt-1 text-xs text-white/45">{helper}</div></div>; }
...
function CompetitionCard({ comp, entered, onJoin }: { comp: CompetitionWithEntries; entered: boolean; onJoin: () => void }) { const entryCount = n(comp.entryCount || (comp.entries || []).length); const maxEntries = n(comp.max_entries || comp.maxEntries); const progress = maxEntries ? Math.min(100, Math.round((entryCount / maxEntries) * 100)) : prizeProgress(comp); const countdown = tournamentCountdown(comp); return <Card className="group relative overflow-hidden rounded-[2rem] border-white/10 bg-slate-950/70 p-0 text-white backdrop-blur-xl shadow-[0_24px_80px_rgba(2,6,23,.28)]"><div className="p-5 space-y-4"><div className="flex items-start justify-between gap-3"><div><Badge className="capitalize bg-primary/20 text-primary-foreground">{comp.tier}</Badge><h3 className="mt-2 text-xl font-black">{comp.name}</h3><p className="text-xs text-white/45">GW {comp.gameWeek} • cutoff {countdown}</p></div>{entered ? <Badge className="bg-emerald-400/20 text-emerald-100">Entered</Badge> : comp.entryOpen === false ? <Badge className="bg-red-400/20 text-red-100"><Lock className="mr-1 h-3 w-3" />Locked</Badge> : <Badge className="bg-cyan-400/20 text-cyan-100">Open</Badge>}</div><PrizeEconomics comp={comp} /><div className="grid grid-cols-2 gap-2 text-sm"><InfoPill label="Entrants" value={`${entryCount}${maxEntries ? `/${maxEntries}` : ""}`} helper="This GW only" /><InfoPill label="Progress" value={`${progress}%`} helper={comp.prizeUnlocked ? "Prize unlocked" : "Prize locked"} /></div><div className="h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-cyan-300" style={{ width: `${progress}%` }} /></div><Button onClick={onJoin} disabled={entered || comp.entryOpen === false} className="w-full rounded-xl bg-primary font-bold">{entered ? "Already Entered" : comp.entryOpen === false ? "Closed" : `Enter N$${money(comp.entryFee)}`}</Button></div></Card>; }
