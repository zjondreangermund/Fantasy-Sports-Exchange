...
function tournamentCountdown(comp: CompetitionWithEntries) {
  const raw = comp.submissionClosesAt;
  if (!raw) return comp.entryOpen === false ? "Locked" : comp.status === "open" ? "Open now" : String(comp.status || "Arena ready");
  const target = new Date(raw).getTime();
  if (!Number.isFinite(target)) return comp.entryOpen === false ? "Locked" : comp.status === "open" ? "Open now" : String(comp.status || "Arena ready");
...
function tournamentSettlementLabel(comp: CompetitionWithEntries) {
  const raw = comp.settlementAt || (comp as any).endDate || (comp as any).end_date;
  if (!raw) return "Tuesday after the gameweek begins";
  const date = new Date(String(raw));
  if (!Number.isFinite(date.getTime())) return "Tuesday after the gameweek begins";
