...
export default function MatchdayQuickDock() {
  ...
  const linkedEvents = useMemo(
    () => events.filter((event) => cards.some((card) => eventMatchesCard(event, card))),
    [cards, events],
  );

  const liveDelta = useMemo(
    () => linkedEvents.reduce((sum, event) => sum + Number(event.delta || 0), 0),
    [linkedEvents],
  );
  ...
  return (
    <Link href="/live-lineup">
      <div className="fixed bottom-4 left-1/2 z-[60] flex -translate-x-1/2 cursor-pointer items-center gap-2 rounded-2xl border border-slate-700/80 bg-slate-950/92 px-3 py-2 text-white shadow-[0_18px_60px_rgba(0,0,0,.45)] backdrop-blur-xl transition hover:-translate-y-0.5 hover:border-cyan-300/50 md:bottom-5 md:px-4 md:py-3">
        ...
        <div className="hidden min-w-[120px] md:block">
          <p className="text-[10px] font-black uppercase tracking-[.18em] text-slate-500">Matchday</p>
          <p className="text-sm font-black text-white">Live Lineup</p>
        </div>
        <div className="flex items-center gap-1 rounded-xl border border-slate-800 bg-black/30 px-2 py-1.5">
          <Swords className="h-3.5 w-3.5 text-slate-400" />
          <span className="text-xs font-black">{cards.length}/5</span>
        </div>
        <div className={positive ? "flex items-center gap-1 rounded-xl border border-emerald-400/25 bg-emerald-400/10 px-2 py-1.5 text-emerald-300" : "flex items-center gap-1 rounded-xl border border-red-400/25 bg-red-400/10 px-2 py-1.5 text-red-300"}>
          <DeltaIcon className="h-3.5 w-3.5" />
          <span className="text-xs font-black">{positive ? `+${liveDelta}` : liveDelta}</span>
        </div>
        <div className="flex items-center gap-1 rounded-xl border border-slate-800 bg-black/30 px-2 py-1.5 text-slate-300">
          <Activity className="h-3.5 w-3.5 text-yellow-300" />
          <span className="text-xs font-black">{linkedEvents.length}</span>
        </div>
      </div>
    </Link>
  );
