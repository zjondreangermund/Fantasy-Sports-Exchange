...
export function PremiumPage({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <main className={`premium-page relative min-h-full overflow-x-hidden overflow-y-visible px-3 pb-[calc(10rem+env(safe-area-inset-bottom,0px))] pt-4 text-white sm:px-6 lg:px-8 ${className}`} style={{ WebkitOverflowScrolling: "touch", touchAction: "pan-y" }}>
      ...
      <div className="relative mx-auto max-w-7xl space-y-5 pb-[calc(6rem+env(safe-area-inset-bottom,0px))]">{children}</div>
    </main>
  );
}
