...
type Props = { player: PlayerCardData; selected?: boolean; onClick?: () => void; showPrice?: boolean; size?: "sm" | "md"; };
const SIZE = { sm: { width: 160, height: 226, radius: 18 }, md: { width: 206, height: 292, radius: 22 } };

function imageOf(player: PlayerCardData) { return player.image || player.imageUrl || player.photo || player.imageCandidates?.[0] || ""; }
function nameOf(player: PlayerCardData) { return String(player.name || "Unknown Player").toUpperCase(); }
...
export default function CollectionStableCard({ player, selected = false, onClick, showPrice = false, size = "sm" }: Props) {
  ...
  return (
    <button type="button" onClick={onClick} style={{ background: "transparent", border: 0, padding: 0, display: "block" }}>
      <article style={{ width: dim.width, height: dim.height, borderRadius: dim.radius, position: "relative", overflow: "hidden", border: `2px solid ${theme.border}`, background: theme.chrome, boxShadow: selected ? `0 0 0 3px #34d399, ${theme.glow}` : theme.glow, color: "white" }}>
        ...
        <div style={{ position: "absolute", inset: 8, borderRadius: dim.radius - 6, overflow: "hidden", background: "#111827" }}>
          {image ? <img src={image} alt={player.name} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /> : <div style={{ width: "100%", height: "100%", display: "grid", placeItems: "center", fontWeight: 900, fontSize: 34, color: "rgba(255,255,255,.25)" }}>{nameOf(player).slice(0, 2)}</div>}
        ...
        <div style={{ position: "absolute", top: 12, left: 12, right: 12, display: "flex", justifyContent: "space-between", fontSize: 9, fontWeight: 900, textShadow: "0 2px 4px #000" }}><span>{player.season || "2026-27"}</span><span>{team.slice(0, 3).toUpperCase()}</span></div>
