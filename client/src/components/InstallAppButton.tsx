...
const ANDROID_APK_BASE_URL = "/api/android/install-apk";
const ANDROID_VERSION = "1.1.10";
const ANDROID_SIZE_LABEL = "about 4.5 MB";
const INSTALL_PROMPT_SNOOZE_KEY = "fantasy_arena_android_1_1_10_install_snoozed_until";
...
if (typeof window !== "undefined") {
  window.addEventListener("beforeinstallprompt", (event) => {
    ...
    // Android installs use the signed APK only. Never turn Fantasy Arena's
    // Android Install App button into a Chrome/PWA home-screen shortcut.
...
function fallbackInstallInstructions() {
  ...
  window.alert("Fantasy Arena installation is currently available on Android through the signed APK.");
}
...
export default function InstallAppButton() {
  ...
  const installed = isAndroidBrowser ? isNativeMobileApp() : isInstalledMobileApp();
  ...
  React.useEffect(() => {
    ...
    const timer = window.setTimeout(() => setPromptOpen(true), isAndroidBrowser ? 650 : 1_800);
    return () => window.clearTimeout(timer);
  }, [installed, isAndroidBrowser]);
  ...
  return (
    <>
      ...
      >
        ...
        <span>{installing ? "Downloading…" : "Install App"}</span>
      </button>
      ...
      {promptOpen ? (
        <div className="fixed inset-0 z-[220] flex items-end justify-center bg-black/75 p-3 backdrop-blur-md sm:items-center" role="dialog" aria-modal="true" aria-label="Install Fantasy Arena" data-install-app-popup>
          ...
          <section className="relative z-10 w-full max-w-md overflow-hidden rounded-[1.75rem] border border-cyan-300/25 bg-[#070b18] p-5 text-white shadow-[0_28px_100px_rgba(0,0,0,.72),0_0_50px_rgba(34,211,238,.12)]">
            ...
            <p className="mt-4 text-[10px] font-black uppercase tracking-[.2em] text-cyan-200/70">Fantasy Arena Android App</p>
            <h2 className="mt-1 text-2xl font-black tracking-tight">Install Fantasy Arena</h2>
            <p className="mt-2 text-sm leading-6 text-slate-400">Install the real Fantasy Arena Android app for native notifications and the full app experience.</p>
            {isAndroidBrowser ? (
              <>
                <p className="mt-3 rounded-2xl border border-emerald-300/12 bg-emerald-300/[.05] px-3 py-2 text-xs text-emerald-100">Android APK {ANDROID_VERSION} · {ANDROID_SIZE_LABEL}</p>
                <p className="mt-2 text-xs leading-5 text-amber-100/80">This installs the signed Android app — not a Chrome home-screen shortcut.</p>
              </>
            ) : null}
            <div className="mt-5 grid grid-cols-2 gap-2">
              ...
              <button type="button" onClick={install} disabled={installing} className="h-11 rounded-2xl bg-gradient-to-r from-cyan-300 via-sky-300 to-violet-400 px-3 text-sm font-black text-slate-950 shadow-[0_0_30px_rgba(34,211,238,.25)] disabled:opacity-60">{installing ? "Downloading…" : "Install APK"}</button>
            </div>
          </section>
        </div>
      ) : null}
