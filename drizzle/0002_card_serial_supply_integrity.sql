-- Canonicalize existing card serials before adding database enforcement.
WITH ranked AS (
  SELECT
    pc.id,
    pc.player_id,
...
SET
  serial_number = ranked.serial_number,
  max_supply = CASE pc.rarity::text
  ...
  END,
  serial_id = concat(
    coalesce(nullif(ranked.initials, ''), 'PLY'), '-',
    pc.player_id, '-',
    upper(left(pc.rarity::text, 1)), '-',
    lpad(ranked.serial_number::text, 4, '0')
  )
FROM ranked
...
BEGIN
  ...
  -- Serialize every mint for the same player and rarity.
  ...
  NEW.serial_id := concat(
    coalesce(nullif(player_initials, ''), 'PLY'), '-',
    NEW.player_id, '-',
    upper(left(NEW.rarity::text, 1)), '-',
...
DROP TRIGGER IF EXISTS player_cards_serial_supply_guard ON app.player_cards;
CREATE TRIGGER player_cards_serial_supply_guard
BEFORE INSERT ON app.player_cards
FOR EACH ROW
EXECUTE FUNCTION app.enforce_player_card_serial_supply();
