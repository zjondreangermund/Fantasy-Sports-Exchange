-- Withdrawal holds, payout confirmation, failed-attempt recovery, and global payout reference integrity.
ALTER TYPE app.withdrawal_status ADD VALUE IF NOT EXISTS 'failed';
...
CREATE UNIQUE INDEX IF NOT EXISTS withdrawal_requests_hold_transaction_unique
  ON app.withdrawal_requests (hold_transaction_id)
  WHERE hold_transaction_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS withdrawal_requests_verification_token_unique
  ON app.withdrawal_requests (verification_token)
  WHERE verification_token IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS withdrawal_payout_reference_unique
  ON app.withdrawal_payout_attempts (payout_reference_key)
  WHERE payout_reference_key IS NOT NULL;

CREATE INDEX IF NOT EXISTS withdrawal_payout_attempts_withdrawal_idx
  ON app.withdrawal_payout_attempts (withdrawal_id, attempt_no DESC);

CREATE INDEX IF NOT EXISTS withdrawal_requests_status_created_idx
  ON app.withdrawal_requests (status, created_at DESC, id DESC);

-- Link legacy requests to their original hold ledger row without guessing across users.
WITH candidates AS (
