...
// 4) All enriched card payloads expose current-gameweek Fantasy Arena points.
{
  ...
  const legacyPositionPrecedence = [
    "    const currentPosition = ",
    "apiFootballPlayer?.position || ",
    "canonical?.position || ",
    'String(player.position || "") || "MID";',
  ].join("");
  const officialPositionPrecedence = '    const currentPosition = canonical?.position || String(player.position || "") || apiFootballPlayer?.position || "MID";';
  source = source.replace(legacyPositionPrecedence, officialPositionPrecedence);
  source = replaceOnce(
    source,
...
// 8) Card profiles must highlight the same precise Arena score shown on cards
// and leaderboards instead of mislabeling season-long FPL points as card points.
{
  const rel = "client/src/components/cards/CardProfileModal.tsx";
  let source = read(rel);
  source = replaceOnce(
    source,
    '  const totalPointsLabel = data.source === "fpl-live" ? "FPL Points" : data.source === "api-football" ? "Arena Score" : "Points";',
    '  const currentGameweekArenaPoints = Number((card as any).currentGameweekPoints ?? (card.player as any)?.currentGameweekPoints ?? 0);\n  const totalPointsLabel = "Arena GW Points";\n  const arenaPointsDisplay = identityVerified && Number.isFinite(currentGameweekArenaPoints)\n    ? currentGameweekArenaPoints.toLocaleString(undefined, { maximumFractionDigits: 4 })\n    : "—";',
    "Card profile current-gameweek Fantasy Arena score",
    'const totalPointsLabel = "Arena GW Points";',
  );
  source = replaceOnce(
    source,
    '<HeroStat icon={<Star className="h-4 w-4" />} label={totalPointsLabel} value={officialStat(data, data.stats.totalPoints)} />',
    '<HeroStat icon={<Star className="h-4 w-4" />} label={totalPointsLabel} value={arenaPointsDisplay} />',
...
// 9) Prize Vault should visibly roll over while left open across a gameweek boundary.
{
