...
patchFile("server/services/apiFootballSync.ts", (original) => {
  ...
  const proJobs = lines(
    ...
    '      `);',
    '      records += 1;',
    '      // STARTING_XI_PLAYER_NOTIFICATIONS_V1',
    '      const starters = Array.isArray(lineup?.startXI) ? lineup.startXI : [];',
    '      for (const starterRow of starters) {',
    '        const starter = starterRow?.player || starterRow || {};',
    '        const apiPlayerId = Number(starter?.id || 0);',
    '        const playerName = String(starter?.name || "").trim();',
    '        const normalizedName = playerName.toLowerCase().replace(/[^a-z0-9]+/g, "");',
    '        if (!apiPlayerId || !playerName || !normalizedName) continue;',
    ...
    '          join app.api_football_fixtures f on f.api_fixture_id=${fixtureId}',
    "          cross join lateral jsonb_array_elements_text(coalesce(ce.lineup_card_ids, '[]'::jsonb)) selected(card_id)",
    '          join app.player_cards pc on pc.id=selected.card_id::integer',
    '          join app.players p on p.id=pc.player_id',
    "          where c.status::text in ('open','live')",
    '            and f.kickoff_at between c.start_date and c.end_date',
    "            and regexp_replace(lower(coalesce(p.name,'')), '[^a-z0-9]+', '', 'g')=${normalizedName}",
    '        `));',
    '        for (const recipient of recipients) {',
    '          await createNotificationOnce(db, {',
    '            userId: String(recipient.userId),',
    '            title: `⚽ ${playerName} starts!`,',
    '            message: `${playerName} is confirmed in the starting XI for ${String(lineup?.team?.name || "the upcoming Premier League match")}. Your submitted Fantasy Arena player is live. Follow the match and your tournament score in Play.`,',
    '            dedupeKey: `fixture:${fixtureId}:starter:${apiPlayerId}`,',
    '          });',
    '        }',
    '      }',
    '    }',
