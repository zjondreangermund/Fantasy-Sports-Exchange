...
if (!source.includes(marker)) {
  ...
  // Keep the original onSuccess block intact so the existing GW4 promo build patch can
  // safely add its post-signup redirect without either patch owning the other's logic.
  const newHandler = `  const handleConfirm = useCallback(() => {\n    if (confirmInFlightRef.current || chooseMutation.isPending) return;\n    const ids = Array.from(selectedPlayerIds);\n    if (ids.length !== 5) return;\n\n    confirmInFlightRef.current = true;\n    setConfirmStatus("submitting");\n\n    chooseMutation.mutate(ids, {\n      onSuccess: () => {\n        setStep("done");\n        refetch();\n      },\n      onError: async (error) => {\n        try {\n          const refreshed = await refetch();\n          if (refreshed.data?.completed) {\n            setStep("done");\n            return;\n          }\n        } catch {}\n        console.error("Starter Draft confirmation failed:", error);\n        confirmInFlightRef.current = false;\n        setConfirmStatus("error");\n      },\n    });\n  }, [selectedPlayerIds, chooseMutation, refetch]);`;
