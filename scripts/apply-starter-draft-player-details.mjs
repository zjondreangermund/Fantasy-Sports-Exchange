...
const MARKER = "STARTER_DRAFT_PLAYER_DETAILS_V1";
...
if (!source.includes(MARKER)) {
  ...
  source = replaceAny(
    ...
    '                          <div className="mt-1.5 flex min-h-[52px] w-full flex-col items-center px-1 text-center">\n                            <span className="block h-6 w-full overflow-hidden text-[9px] font-black leading-3 text-white/90 sm:text-[10px]">{card.player?.name || "Player"}</span>\n                            <div className="mt-1 flex w-full items-start justify-center gap-1">\n                              <span className="shrink-0 rounded-full border border-cyan-200/20 bg-cyan-300/10 px-1.5 py-0.5 text-[7px] font-black uppercase tracking-[0.08em] text-cyan-100 sm:text-[8px]">{String(card.player?.position || "N/A").toUpperCase()}</span>\n                              <span title={String(card.player?.team || "Premier League")} className="block max-h-[20px] min-w-0 overflow-hidden text-left text-[7px] font-bold uppercase leading-[10px] text-white/55 sm:text-[8px]">{card.player?.team || "Premier League"}</span>\n                            </div>\n                          </div>',
    "player name/team/position block",
  );

  // The optional reveal step also uses the signup cards. Hide game stats there,
  // while keeping CardThumbnail's own position + club metadata visible.
  ...
  console.log("[starter-player-details] Starter Draft now shows player name, canonical position and Premier League club without card-stat overlap.");
} else {
  console.log("[starter-player-details] Starter Draft player details already ready.");
}

// Guard the data contract as well as the presentation: signup offers must still
// come from the current FPL Premier League team list, and FPL element_type must
// remain the source of GK/DEF/MID/FWD rather than stale local labels.
