...
const reconciledResponse = to.replace(
  "        const normalized = normalizeCompetitionRow({ ...comp, entryCount: entries.length });",
  "        const recordedEntrantRevenue = toMoney(entries.reduce((total: number, entry: any) => total + Number(entry.entryFeePaid ?? entry.entry_fee_paid ?? 0), 0));\n        const normalized = normalizeCompetitionRow({ ...comp, entryCount: entries.length, currentEntrantRevenue: recordedEntrantRevenue });",
);
fs.writeFileSync(file, source.replace(from, reconciledResponse));
