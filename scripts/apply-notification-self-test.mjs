...
patchFile("server/routes/notifications.routes.ts", (input) => {
  let source = input;
  const helperAnchor = `function minutesUntil(value: unknown): number | null {
...
`;
  ...
  const route = `  app.post("/api/push/test", requireAuth, async (req: any, res) => {
    try {
...
patchFile("client/src/components/PushNotificationControl.tsx", (input) => {
  let source = input;
  const typeAnchor = `type EnabledDevice =
...
`;
  ...
  const testMutation = `  const testMutation = useMutation({
