...
// API-Football is primary, but an exact/strong name is never permission to
// change a goalkeeper into an outfield player (or vice versa). Only a valid
// canonical position is enforced; legacy/unknown position strings stay unset.
{
  ...
  source = replaceRequired(
    ...
    "API-Football cross-position name bypass removal",
  );
  write(file, source);
}
...
// a different position. This prevents the exact GK-card / MID-label split seen
// in the tournament builder while server data is being reconciled.
{
