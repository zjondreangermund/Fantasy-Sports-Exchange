...
export async function applyMarketplaceTradeLedger(tx: any, input: {
  buyerId: string;
  sellerId: string;
  cardId: number;
  price: number;
  ...
  const feeRate = Number.isFinite(Number(input.feeRate)) ? Number(input.feeRate) : 0.08;
  const fee = toMoney(price * feeRate);
  const sellerReceives = toMoney(price - fee);

  if (!input.buyerId || !input.sellerId || !Number.isInteger(input.cardId) || input.cardId <= 0 || price <= 0) {
    throw new Error("Invalid marketplace trade ledger input");
  }
  ...
  const [debitedBuyer] = await tx
    ...
    .where(and(eq(wallets.userId, input.buyerId), sql`${wallets.balance} >= ${price}`))
    .returning();
  ...
  const [creditedSeller] = await tx
    .update(wallets)
    .set({ balance: sql`${wallets.balance} + ${sellerReceives}` } as any)
    .where(eq(wallets.userId, input.sellerId))
    .returning();
  ...
  await tx.insert(transactions).values([
    {
      ...
      amount: -price,
      description: `marketplace card:${input.cardId} buyer:${input.buyerId} seller:${input.sellerId} gross:${price.toFixed(2)}`,
    } as any,
    {
      userId: input.sellerId,
      type: "marketplace_sale",
      amount: sellerReceives,
      description: `marketplace card:${input.cardId} buyer:${input.buyerId} seller:${input.sellerId} gross:${price.toFixed(2)} fee:${fee.toFixed(2)}`,
    } as any,
  ]);

  return { buyerWallet: debitedBuyer, sellerWallet: creditedSeller, fee, sellerReceives };
}


export async function refundWalletHold(tx: any, input: {
  userId: string;
  amount: number;
}) {
  const amount = toMoney(input.amount);
  if (!input.userId || amount <= 0) {
    throw new Error("Invalid wallet hold refund input");
  }

  const [refundedWallet] = await tx
    .update(wallets)
    .set({
      balance: sql`${wallets.balance} + ${amount}`,
      lockedBalance: sql`${wallets.lockedBalance} - ${amount}`,
  ...
  if (!refundedWallet) {
    throw new Error("Locked funds not available for refund");
  }

  return refundedWallet;
}

export async function settleHeldAuctionBid(tx: any, input: {
  winnerId: string;
  sellerId: string;
  cardId: number;
  amount: number;
  ...
  const amount = toMoney(input.amount);
  const feeRate = Number.isFinite(Number(input.feeRate)) ? Number(input.feeRate) : 0;
  const fee = toMoney(amount * feeRate);
  const sellerReceives = toMoney(amount - fee);

  if (!input.winnerId || !input.sellerId || !Number.isInteger(input.cardId) || input.cardId <= 0 || amount <= 0) {
    throw new Error("Invalid auction settlement ledger input");
  }
  if (input.winnerId === input.sellerId) {
    throw new Error("Cannot settle auction to seller");
  }
  if (sellerReceives < 0) {
    throw new Error("Invalid auction fee calculation");
  }

  const [winnerWallet] = await tx
    .update(wallets)
    .set({ lockedBalance: sql`${wallets.lockedBalance} - ${amount}` } as any)
    .where(and(eq(wallets.userId, input.winnerId), sql`${wallets.lockedBalance} >= ${amount}`))
    .returning();

  if (!winnerWallet) {
    throw new Error("Winning bid hold is not available");
  }
  ...
  await tx.insert(transactions).values([
    {
      ...
      amount: -amount,
      description: `Auction won: Card #${input.cardId} gross:${amount.toFixed(2)}`,
    } as any,
    {
      userId: input.sellerId,
      type: "auction_settlement",
      amount: sellerReceives,
      description: `Auction sale: Card #${input.cardId} gross:${amount.toFixed(2)} fee:${fee.toFixed(2)}`,
    } as any,
  ]);

  return { winnerWallet, sellerWallet, fee, sellerReceives };
}
