...
import { auditLogs, playerCards, players, transactions, wallets, withdrawalRequests } from "../../shared/schema.js";
import {
  getLiquidityScore,
  getPrimarySalePrice,
...
export function registerRetentionRoutes(app: Express, deps: { requireAuth: any; storage: IStorage }) {
  ...
  app.get("/api/forge/options", requireAuth, async (req: any, res) => {
    try {
      ...
      const options = Array.from(grouped.values())
        .filter((list) => list.length >= COMMON_BURN_COUNT)
        .map((list) => {
      ...
      return res.json({
        options,
        rules: {
          samePlayerRequired: true,
          burnCount: COMMON_BURN_COUNT,
          fee: COMMON_TO_RARE_BURN_FEE,
          fromRarity: "common",
          toRarity: "rare",
        },
  ...
  app.post("/api/forge/burn-same-player", requireAuth, async (req: any, res) => {
    ...
    if (cardIds.length !== COMMON_BURN_COUNT) {
      return res.status(400).json({ message: `Exactly ${COMMON_BURN_COUNT} common cards are required` });
    }
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        if (!wallet || toMoney(wallet.balance || 0) < COMMON_TO_RARE_BURN_FEE) {
          throw new Error(`N$${COMMON_TO_RARE_BURN_FEE.toFixed(2)} is required to forge a rare card`);
        }

        const [existingRare] = await tx
          .select()
          .from(playerCards)
          .where(and(eq(playerCards.ownerId, userId), eq(playerCards.playerId, Number(first.playerId)), eq(playerCards.rarity, "rare" as any)));
        ...
        await tx
          .update(playerCards)
          .set({ ownerId: null as any, forSale: false, price: 0 } as any)
          .where(sql`${playerCards.id} = ANY(${cardIds})` as any);

        const minted = await storage.createPlayerCard({
          ownerId: userId,
          playerId: Number(first.playerId),
          rarity: "rare",
          forSale: false,
          price: 0,
          decisiveScore: 42,
        ...
        await tx.insert(transactions).values({
          userId,
          type: "burn_fee",
          amount: -COMMON_TO_RARE_BURN_FEE,
          grossAmount: COMMON_TO_RARE_BURN_FEE,
          feeAmount: COMMON_TO_RARE_BURN_FEE,
          netAmount: -COMMON_TO_RARE_BURN_FEE,
          sourceType: "forge_burn",
          status: "completed",
          description: `Forge burn fee for player:${first.playerId} using cards:${cardIds.join(",")}`,
        } as any);
        ...
        await tx.insert(transactions).values({
          ...
          feeAmount: 0,
          netAmount: 0,
          sourceType: "forge_mint",
          status: "completed",
          description: `Forged rare player:${first.playerId} mintedCard:${minted.id} from commons:${cardIds.join(",")}`,
        } as any);

        await tx.insert(auditLogs).values({
          userId,
          action: "forge.common_to_rare",
          meta: {
            playerId: first.playerId,
            burnedCardIds: cardIds,
            mintedCardId: minted.id,
            fee: COMMON_TO_RARE_BURN_FEE,
          },
        } as any);
  ...
  app.get("/api/retention/summary", requireAuth, async (req: any, res) => {
    ...
    return res.json({
      missions,
      nextBestAction: forgeReadyCount > 0
        ? { key: "forge", title: "Forge a rare duplicate upgrade", ctaPath: "/card-lab" }
        : { key: "market", title: "Scout underpriced cards", ctaPath: "/marketplace" },
      deadline: nextCompetition || null,
      economy: { forgeReadyCount, forgeCost: COMMON_TO_RARE_BURN_FEE, forgeRequirement: COMMON_BURN_COUNT },
    });
  });
}
