...
function requestKey(req: any, userId: string, action: string): string {
  const raw = String(req.headers?.["x-idempotency-key"] || req.body?.idempotencyKey || "")
    .trim()
    .slice(0, 120);
  return raw ? `${userId}:${action}:${raw}` : "";
}
  ...
  app.post("/api/wallet/deposit", requireAuth, async (req: any, res) => {
    try {
      ...
      const existing = await db
        .select()
        .from(transactions)
        .where(and(eq(transactions.userId, userId), eq(transactions.externalTransactionId, externalTransactionId)))
        .limit(1);
      ...
      await db.insert(auditLogs).values({
        userId,
        action: "wallet.deposit.submitted",
        meta: { transactionId: created.id, amount, paymentMethod, externalTransactionId },
      } as any);
  ...
  app.post("/api/wallet/withdraw", requireAuth, async (req: any, res) => {
    try {
      ...
      if (amount < MIN_WITHDRAWAL_AMOUNT) {
        return res.status(400).json({ message: `Minimum withdrawal is N$${MIN_WITHDRAWAL_AMOUNT.toFixed(2)}` });
      }
      if (!paymentMethod) return res.status(400).json({ message: "Payment method required" });
      if ((paymentMethod === "ewallet" || paymentMethod === "mobile_money") && (!req.body?.ewalletProvider || !req.body?.ewalletId)) {
        return res.status(400).json({ message: "eWallet provider and destination required" });
      }
      if (paymentMethod !== "ewallet" && paymentMethod !== "mobile_money" && (!req.body?.accountHolder || (!req.body?.accountNumber && !req.body?.iban))) {
        return res.status(400).json({ message: "Account holder and bank account details required" });
      }
      ...
      const result = await db.transaction(async (tx) => {
        ...
        if (knownKey) {
          const [existing] = await tx.select().from(withdrawalRequests)
            .where(and(eq(withdrawalRequests.userId, userId), eq(withdrawalRequests.verificationToken, key)))
            .limit(1);
        ...
        await tx.insert(auditLogs).values({
          ...
          meta: { withdrawalId: withdrawal.id, amount, fee, netAmount, paymentMethod },
        } as any);
    ...
    try {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        const [updated] = await tx.update(withdrawalRequests).set({
          ...
          reviewedAt: new Date(),
        } as any).where(eq(withdrawalRequests.id, withdrawalId)).returning();
        ...
        await tx.insert(auditLogs).values({
          userId,
          action: `wallet.withdrawal.${nextStatus}`,
          meta: { withdrawalId, amount, adminId, adminNotes },
        } as any);
